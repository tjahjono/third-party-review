...
// Enrolment is what the user needs to add the account to their authenticator.
type Enrolment struct {
	// Secret is the base32 shared secret, kept in the form only until the
	// user confirms a code from it.
	Secret string
...
func (s *Service) BeginMFAEnrolment(ctx context.Context, userID int64) (*Enrolment, error) {
...
func (s *Service) CompleteMFAEnrolment(ctx context.Context, userID int64, secret, code string) ([]string, error) {
	user, err := s.users.GetByID(ctx, userID)
	if err != nil {
		return nil, err
	}
	if user.MFAEnabled {
		return nil, domain.ValidationError{Field: "mfa", Message: "Two-factor authentication is already switched on."}
	}
	if strings.TrimSpace(secret) == "" {
		return nil, domain.ValidationError{Field: "mfa", Message: "The enrolment expired. Start again."}
	}

	ok, err := totp.Validate(secret, code, time.Now(), totp.DefaultSkew)
	if err != nil {
		return nil, domain.ValidationError{Field: "code", Message: "That secret could not be read. Start the enrolment again."}
	}
	if !ok {
		return nil, domain.ValidationError{
			Field:   "code",
...
func (s *Service) DisableMFA(ctx context.Context, userID int64, password string) error {
	user, err := s.users.GetByID(ctx, userID)
	if err != nil {
		return err
	}
	...
	if err := bcrypt.CompareHashAndPassword([]byte(user.PasswordHash), []byte(password)); err != nil {
		return domain.ValidationError{Field: "password", Message: "That password is not correct."}
	}
	...
	if err := s.users.ReplaceRecoveryCodes(ctx, userID, nil); err != nil {
		return err
	}
...
func (s *Service) RegenerateRecoveryCodes(ctx context.Context, userID int64, password string) ([]string, error) {
	...
	if !user.MFAEnabled {
		return nil, domain.ValidationError{Field: "mfa", Message: "Two-factor authentication is not switched on."}
	}
	if err := bcrypt.CompareHashAndPassword([]byte(user.PasswordHash), []byte(password)); err != nil {
		return nil, domain.ValidationError{Field: "password", Message: "That password is not correct."}
	}
	codes, hashes, err := generateRecoveryCodes(recoveryCodeCount)
	if err != nil {
		return nil, err
...
func (s *Service) consumeRecoveryCode(ctx context.Context, userID int64, code string) (bool, error) {
	...
	hashes, err := s.users.RecoveryCodes(ctx, userID)
	if err != nil {
		return false, err
	}
	for _, h := range hashes {
		if bcrypt.CompareHashAndPassword([]byte(h), []byte(normalized)) == nil {
			if err := s.users.ConsumeRecoveryCode(ctx, userID, h); err != nil {
				return false, err
			}
			return true, nil
		}
...
func EnrolmentFor(secret, username string) *Enrolment {
